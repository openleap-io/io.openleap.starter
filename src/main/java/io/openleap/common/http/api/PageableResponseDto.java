 ...
 */
package io.openleap.common.http.api;
...
public record PageableResponseDto<T>(
     ...
     */
    public static <T> PageableResponseDto<T> of(
            List<T> content,
            long totalElements,
            int totalPages,
            int pageNumber,
            int pageSize,
            boolean last) {
        return new PageableResponseDto<>(content, totalElements, totalPages, pageNumber, pageSize, last);
    }
}
